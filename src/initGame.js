...
export default function initGame() {
    ...
    k.loadSprite("characters", "./characters.png", {
        sliceY: 2, // ? How many rows per column
        sliceX: 8, // ? How many columnns per row, just look at the image man
        anims: {
            "down-idle": 0,
            "up-idle": 1,
            "right-idle": 2,
    ...
    const player = k.add([
        ...
        k.area(), // ? hitbox
        k.body(), // ? physic stuff enable
        k.anchor("center"),  // ? draw at the center start at the center i guess
        k.pos(k.center()), // ? POS AT CENTER I GUESS
        k.scale(8),
        "player", // ? tag
        {
            speed: 800,
    ...
    npc.onCollide("player", (player) => {
        ...
        if (player.direction.eq(k.vec2(1, 0))) {
            store.set(textBoxContentAtom, "Made with React.js!");
            npc.play("npc-left")
        }

        if (player.direction.eq(k.vec2(-1, 0))) {
            store.set(textBoxContentAtom, "Poop");
            npc.play("npc-right");
        }
